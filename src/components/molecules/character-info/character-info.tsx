...
export const CharacterInfo: React.FC<CharacterInfoProps> = ({id, name, surname, role, description}) => {
    ...
    return(
        <View style={tw`flex flex-row mt-8 pb-6 border-b border-b-slate-300`}>
          <ImageBackground source={{uri: `data:image/jpeg;base64,${characterRef.data?.data}`}} resizeMode="cover" style={tw`h-35 w-27 rounded`} imageStyle={tw`rounded`}>
            ...
            <View style={tw`flex px-4 w-[70%]`}>
              ...
              <TouchableOpacity style={tw`bg-yellow-500 self-start ml-auto py-1.5 px-4 rounded-lg mt-auto`} onPress={()=> navigation.navigate("CharacterContent", {id: id })}>
